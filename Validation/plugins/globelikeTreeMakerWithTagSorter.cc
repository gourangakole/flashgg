#include "globelikeTreeMakerWithTagSorter.h"
...
FlashggTreeMakerWithTagSorter::FlashggTreeMakerWithTagSorter( const edm::ParameterSet &iConfig ):
   ...
   // 06.09.2017
    //weightTokenDummy_( consumes<edm::MergeableDouble>(iConfig.getUntrackedParameter<edm::InputTag>("weightsCount"))),
    ...
    puMC_  ( iConfig.getParameter<std::vector<double> > ( "mcPu" ) )
{
    ...
    totalGenWeight_ = 0.;
weightTokenDummy_ = consumes<edm::MergeableDouble, edm::InLumi>(edm::InputTag("weightsCount","totalWeight"));
...
void FlashggTreeMakerWithTagSorter::endLuminosityBlock(edm::LuminosityBlock const & iLumi, edm::EventSetup const& iSetup) {
    ...
    iLumi.getByToken(weightTokenDummy_,totWeight);//weightsCount","totalWeight"), totWeight);
    if (totWeight.isValid()) 
        totalGenWeight_ += (double)totWeight->value; 
    
    //totalGenWeight_=1;// += (double)totWeight->value;
}
